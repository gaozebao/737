...
intentsCatcher = /([0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}) I\/ActivityManager\(\s..[0-9]*\): Starting activity: Intent \{( act=(android\.intent\.action\.[A-Z]+))?( cat=(\[android.intent.category.[A-Z]+\]))?( dat=([A-z]|[0-9]|\.|\/|:)*)?( flg=(0x([0-9]|[a-f])*))?( cmp=(([A-z]|[0-9]|\.|\/|:)*))?( bnds=(\[[0-9]+,[0-9]+\]\[[0-9]+,[0-9]+\]))? (.*)?\}[0-9]*/
